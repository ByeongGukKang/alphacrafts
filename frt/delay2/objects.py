...
class Trader(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        if list(inspect.signature(signal_func).parameters.keys()) != ['time', 'packet_data']:
            raise Exception('Signal Function must have two parameters (time, packet_data)')
        if list(inspect.signature(order_func).parameters.keys()) != ['time', 'signals', 'current_price', 'cash_balance', 'pf_value', 'positions']:
            raise Exception('Order Function must have six parameters (time, signals, current_price, cash_balance, pf_value, positions)')
...
class Exchange:
    ...
    def run(self): # 여기서 for iter 돌면서 테스트 이루어짐
        ...
        for iter, iter_time in enumerate(tqdm(self.SENDER.price_index, desc=f'Backtesting [{self.TRADER.name}]')):
            ...
            self.total_time = self.total_time + (datetime.now() - str_time)
